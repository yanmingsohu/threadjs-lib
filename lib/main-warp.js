...
var bootnode = __dirname + '/bootnode.js';
...
function warp(native) {
  ...
  function create_node(code, filename, libs) {
    if (!code) {
      code = fs.readFileSync(filename, 'utf8');
    }

    var bootcode   = fs.readFileSync(bootnode, 'utf8');
    var th         = _create(bootcode, bootnode, libs, true, bootnode);
    var _send      = th.send;
    var send_queue = [];

    th.once('thread_ready', function() {
      _send('eval_code', {
        code : code,
        file : filename,
      })
    ...
    th.once('code_running', function() {
      th.send = _send;
      for (var i=0, e=send_queue.length; i<e; ++i) {
        _send.apply(th, send_queue[i]);
      }
      send_queue = null;
    });

    th.send = function save_to_queue() {
      send_queue.push(arguments);
    };
  ...
  function create(code, filename, libs) {
    ...
    return _create(code, filename, libs);
  }


  function _create(code, filename, libs, _n, _0) {
    thread_code_warp.set('LIB_SCRIPT', exports_libs_code(libs));
    thread_code_warp.set('USE_SCRIPT', code);

    filename         = path.normalize(filename);
    var main_context = native.create(thread_code_warp.code(), filename, _n, _0);
